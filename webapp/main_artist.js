var edgesFilePath = 'data/song_similarity_recommendations.csv';
var nodesFilePath = 'data/songnodes.csv';
...
Promise.all([
    d3.dsv(",", edgesFilePath, function (edge) {
        return {
            source: edge.source_song_id,
            target: edge.target_song_id,
            priority: parseInt(edge.rank)
        };
    }),
    d3.dsv(",", nodesFilePath, (node) => {
        return {
            artist_id: node.song_id,
            artist_name: node.song_name,
